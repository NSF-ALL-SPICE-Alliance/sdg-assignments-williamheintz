...
```{r}
ggplot(data = sdr_data, aes(x= goal_8_score, y = goal_9_score, color = goal__score)) +
  geom_point()
```
...
```{r}
ggplot(data = sdr_data, aes(x= goal_8_score, y = goal_9_score, color = goal_10_score)) +
  geom_point()
```
In this plot we see there is a cluster of countries that have high goal 10 scores as well as high goal 8 and 9 scores, hence the cluster of light dots in the top right corner. Also, you'll notice that the majority of countries that have low goal 10 scores are around midrange with their goal 8 and 9 scores. ```
...
MENA_sdr_data <- sdr_data %>% 
  filter(regions_used_for_the_sdr == "MENA") # filtering for LAC countries only
```
