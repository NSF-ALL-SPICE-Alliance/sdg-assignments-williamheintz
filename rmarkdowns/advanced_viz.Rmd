...
ggplot(sdr_data, aes(x = goal_4_score, fill=regions_used_for_the_sdr)) +
  ...
  labs(title = "Supah Awesome Data",
       x = "SDG 4 Score",
       y = "Supah Awesome Countries",
       fill = "Region")
...
```{r}
goal_4_point <- ggplot(sdr_data, aes(x = goal_4_score,y = goal_3_score, color=regions_used_for_the_sdr)) +
  geom_point() +
  theme_minimal() +
  scale_fill_viridis_d() +
  labs(title = "goal 4 scores",
       x = "SDG 4 Score",
...
```{r, warning = FALSE}
goal_4_point <- ggplot(data = sdr_data, aes(x= goal_3_score, y = goal_4_score, color = regions_used_for_the_sdr)) +
  geom_point() +
  theme_minimal() +
...
mytext <- paste(
    "Country: ", sdr_data_world_joined$country,"<br/>", 
    "Goal 5 Score: ", round(sdr_data_world_joined$goal_5_score, 2), 
    sep="") %>%
  lapply(htmltools::HTML)
...
leaflet(sdr_data_world_joined) %>% 
  addTiles()  %>% 
  setView( lat=10, lng=0 , zoom=2) %>%
  addPolygons(stroke = FALSE, fillOpacity = 0.5, smoothFactor = 0.5, color = ~colorQuantile("YlOrRd", goal_5_score)(goal_5_score), label = mytext)
```
