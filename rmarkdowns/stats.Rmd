...
```{r, warning=FALSE, message=FALSE}
sdg_scores_2023 <- ggplot(sdr_data, aes(x = goal_3_score, 
                     y = goal_8_score, label = country)) + 
  geom_point() +
  geom_smooth() +
...
ggplotly(sdg_scores_2023)
```
In this scatterplot we see a strong positive correlation. The positive correlation in this scatterplot is for goal 3 scores ("Good health and well being) and goal 8 scores (Decent work and economic growth).Afghanistan is definitely and outlier with very low goal 8 and 3 scores.It makes sense that the countries with more decent work (goal 8) also have a higher percentage of good health and well being (goal 3).
